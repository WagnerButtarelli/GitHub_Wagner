...
var CONFIG = {
  SPREADSHEET_ID: '1YPcIOw-_m-ZH71_WLB0MYYWemtm1sh6sf1OXcyLL2Rg',
  ABA_ESTOQUE: 'Estoque',
  ABA_VENDAS: 'Vendas',
  ABA_ACESSOS: 'acessos',
...
function inicializarSistema() {
  obterAba_(CONFIG.ABA_ESTOQUE, CABECALHO_ESTOQUE);
  ...
  // Cria usuário padrão caso a aba esteja vazia
...
function buscarProdutoPorCodigo(codigo) {
  ...
  if (nomeExterno) {
    return { encontrado: true, origem: 'openfoodfacts', nome: nomeExterno, valorCompra: '', valorVenda: '' };
  }
...
function buscarNomeOpenFoodFacts_(codigo) {
  try {
    var url = 'https://world.openfoodfacts.org/api/v0/product/' + encodeURIComponent(codigo) + '.json';
...
function buscarProdutoParaVenda(codigo) {
  ...
  if (lotes.length === 0) return { encontrado: false };

  lotes.sort(function (a, b) { return a.validade - b.validade; }); // FIFO
